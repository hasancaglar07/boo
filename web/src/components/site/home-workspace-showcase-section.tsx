...
const workspacePointIcons = [Target, Zap, Clock] as const;

const workspaceAssetBooks = getSeededSiteBooks("home-workspace-cover-scene", 3);

export function HomeWorkspaceShowcaseSection() {
  const t = useTranslations("HomeWorkspaceShowcase");

  const workspacePoints = [0, 1, 2].map((i) => ({
    icon: workspacePointIcons[i],
        ...
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild size="lg">
              <a href="/start/topic">{t("startFirstBook")}</a>
            </Button>
          ...
          <p className="mt-3 text-xs text-muted-foreground/70">
            {t("trustNote")} · {NO_API_COST_CLAIM} · {KDP_GUARANTEE_CLAIM}
          </p>
        </div>
