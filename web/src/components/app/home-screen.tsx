...
export function HomeScreen() {
  ...
  function shareWhatsApp() {
    ...
    const text = `Write professional books in minutes with BookGenerator.net! ${affiliateData.referralUrl}`;
  ...
  if (viewer && !hasNamedProfilee) {
    onboardingActions.push({
      ...
      description: "Display your real name in the library and workspace.",
  ...
  if (viewer && !hasGoal) {
    onboardingActions.push({
      icon: Target,
      label: "Add a writing goal",
      description: "Align wizard defaults and product tone with your goal.",
      run: () => router.push("/app/settings/profile"),
    });
  }
  if (!books.length) {
    onboardingActions.push({
      ...
      description: "Generate your first preview in five short steps and open your library.",
  ...
  if (viewer && !viewer.emailVerified) {
    onboardingActions.push({
      icon: ShieldAlert,
      label: "Verify email",
      description: "Complete email verification for account security and notifications.",
      run: () => router.push("/app/settings/profile"),
    });
  }
  if (latestBook) {
    onboardingActions.push({
      icon: FileText,
      label: "Return to latest preview",
      description: `Go back to the preview and upgrade flow for ${latestBook.title}.`,
  ...
  if (loadingBooks) {
    return (
      <AppFrame current="home" title="My Books" books={[]} viewer={viewer}>
        {/* Hero skeleton */}
        <div className="home-animate-in mb-6 rounded-[28px] border border-primary/15 bg-card p-5 md:p-8" role="status" aria-label="Page loading">
        ...
        <span className="sr-only">Content loading, please wait.</span>
      </AppFrame>
    );
  }
  ...
  return (
    <AppFrame
      ...
      viewer={viewer}
      actions={[
        { label: "Create new book", description: "Open in-app writing flow", run: () => router.push(newBookHref) },
        {
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        <Card className="overflow-hidden border-primary/20 bg-[radial-gradient(circle_at_top_right,_rgba(188,104,67,0.08),_transparent_60%)] transition-shadow hover:shadow-[0_4px_20px_rgba(188,104,67,0.12)]">
          <CardContent className="p-5 md:p-8 lg:p-12">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="bg-primary/10 text-primary border-primary/20">Welcome</Badge>
              <Badge className={viewer?.emailVerified ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20" : "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20"}>
                {viewer?.emailVerified ? "Verified" : "Verification pending"}
              </Badge>
            ...
            <p className="mt-3 max-w-2xl text-base leading-7 text-muted-foreground">
              {latestBook
                ? `Preview, workspace, and export flow for ${latestBook.title} are waiting for you. Pick up where you left off or start a new book.`
                : "Start your first book, manage the preview in the same session, and grow your library from here."}
            </p>
