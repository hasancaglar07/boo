              ...
              {!book.resources?.exports?.length ? (
                <div className="rounded-xl border border-dashed border-border/50 bg-muted/20 p-8 text-center">
                  ...
                  <div className="mt-3 text-sm font-medium text-muted-foreground">No output yet</div>
                  <p className="mt-1 text-xs text-muted-foreground">Click the EPUB or PDF button above to create your first file</p>
                </div>
              ) : null}
            </div>
          </div>
...
[Output exceeded 50000 byte limit (57426 bytes total). Full output saved to C:\Users\ihsan\AppData\Local\Temp\.tmpP8JyT2\stdout-1. Read it with shell commands like `head`, `tail`, or `sed -n '100,200p'` up to 2000 lines at a time.]